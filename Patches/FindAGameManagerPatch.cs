...
namespace BanMod
{
    ...
    internal static class BanModFindAGameActiveLobbyPatch
    {
        ...
        private static float rowStartY;
        private static float rowStartZ = 25f;
        private static float rowSpacing = DEFAULT_ROW_SPACING;
        ...
        private static void Start_Prefix(FindAGameManager __instance)
        {
            ...
            {
                if (__instance == null ||
                    __instance.gameContainers == null ||
                    __instance.gameContainers.Length == 0)
                {
                    return;
                }

                List<GameContainer> originalContainers =
                    __instance.gameContainers
                        .Where(c => c != null && c.transform != null)
                        .OrderByDescending(c => c.transform.localPosition.y)
                        .ToList();

                if (originalContainers.Count == 0)
                    return;

                GameContainer prefab =
                    originalContainers[Math.Min(
                        4,
                        originalContainers.Count - 1)];
                ...
                rowStartX =
                    originalContainers[0].transform.localPosition.x;
                rowStartY =
                    originalContainers[0].transform.localPosition.y;
                rowStartZ = 25f;
                rowSpacing = DetectRowSpacing(originalContainers);

                GameObject list =
                    new GameObject("BanModGameListScroller");
                ...
                scroller.SetYBoundsMax(10.5f);

                BoxCollider2D box =
                    parent.gameObject.GetComponent<BoxCollider2D>();
                ...
                while (containers.Count < MAX_DISPLAYED_LOBBIES)
                {
                    ...
                    PositionContainer(clone, containers.Count);
                    clone.gameObject.SetActive(false);
                    containers.Add(clone);
                }

                if (containers.Count > MAX_DISPLAYED_LOBBIES)
                {
                    containers =
                        containers
                            .Take(MAX_DISPLAYED_LOBBIES)
                            .ToList();
                }
                ...
                cutoff.transform.localPosition =
                    new Vector3(0f, 3f, 1f);
                cutoff.transform.localScale =
                    new Vector3(1500f, 200f, 100f);
            }
            catch (Exception ex)
            {
                Debug.Log(
                    "[BanMod] lobby browser Start patch error: " +
                    ex.Message);
            }
        }
        ...
        private static int RenderFinalList(FindAGameManager manager, List<BanModRow> banModRows, List<GameListing> vanillaGames)
        {
            ...
            for (int i = index; i < manager.gameContainers.Length; i++)
            {
                GameContainer unused = manager.gameContainers[i];
                if (unused == null)
                    continue;

                PositionContainer(unused, i);
                unused.gameObject.SetActive(false);
                unused.transform.localScale = Vector3.one;
                RemoveBanModLabel(unused);
            }
        ...
        private static float DetectRowSpacing(
            List<GameContainer> containers)
        {
            try
            {
                ...
                List<float> ys = containers
                    .Where(c => c != null && c.transform != null)
                    .Select(c => c.transform.localPosition.y)
                    .Distinct()
                ...
                {
                    float difference = Mathf.Abs(ys[i - 1] - ys[i]);

                    if (difference > 0.05f && difference < best)
                        best = difference;
                }
        ...
        private static void PositionContainer(
