...
//        private static string NomeGiocatore(PlayerControl player)
//            => player?.Data?.PlayerName ?? player?.name ?? "Sconosciuto";

//        private static string EtichettaGiocatore(PlayerControl player)
//            => player == null ? "giocatore-nullo" : $"{NomeGiocatore(player)} [pid={player.PlayerId}]";
...
//        private static void LogSoloFile(string testo)
//        {
//            BMLogger.LogDebug(testo, "RPC-VERBOSE");
//        }

//        private static string PulisciTestoLog(string testo)
//        {
//            if (string.IsNullOrEmpty(testo))
//                return string.Empty;

//            return testo.Replace("\r", "\\r").Replace("\n", "\\n");
//        }
...
//            try
//            {
//                int action = Options.ActionSicko.GetValue();
//                return action == 1 ? 1 : 2;
//            }
//            catch
//            {
...
//            => action == 2 ? "bannato" : "espulso";

//        private static string GetRpcNome(int callId, bool hasByteId, byte callIdByte)
//        {
//            if (!hasByteId)
...
//        private static void RegistraRpcRicevuta(PlayerControl player, int callId, bool hasByteId, byte callIdByte, MessageReader reader)
//        {
//            string rpcName = GetRpcNome(callId, hasByteId, callIdByte);
...
//            LogSoloFile(
//                $"RPC ricevuta | Giocatore={EtichettaGiocatore(player)} | ClientId={clientId} | " +
//                $"CallIdInt={callId} | CallIdByte={(hasByteId ? callIdByte.ToString() : "N/D")} | " +
//                $"Nome={rpcName} | Posizione={position} | Lunghezza={length} | Rimanenti={remaining}"
//            );
//        }

//        private static void RegistraMessaggioRicevuto(PlayerControl player, string tipo, string contenuto)
//        {
//            LogSoloFile(
//                $"Messaggio ricevuto | Tipo={tipo} | Giocatore={EtichettaGiocatore(player)} | " +
//                $"Testo={PulisciTestoLog(contenuto)}"
//            );
//        }
...
//            catch (Exception ex)
//            {
//                BMLogger.Warn($"Impossibile mostrare la notifica a schermo: {ex.Message}", LogTag);
//            }
//        }

//        private static void ApplicaAzioneSospetto(
//            PlayerControl player,
//            ClientData client,
//            int cheaterClientId,
//            int action,
//            string motivoBan,
//            string messaggioSchermo,
//            bool aggiungiBan = true,
//            bool aggiungiCheater = true,
//            bool inviaWarning = true)
//        {
//            if (player == null)
//                return;
...
//            string nome = NomeGiocatore(player);
//            bool firstHandle = AlreadyHandledCheaters.Add(playerId);

//            BMLogger.Warn(
//                $"Azione anti-cheat applicata | Giocatore={nome} | ClientId={cheaterClientId} | Action={action} | Motivo={motivoBan}",
//                LogTag);

//            ShowScreenMessage(messaggioSchermo);
...
//                        BMLogger.Info($"Giocatore aggiunto alla lista cheater: {nome} | ClientId={cheaterClientId}", LogTag);
//                    }
//                }
//                catch (Exception ex)
//                {
//                    BMLogger.Error($"Errore durante l'aggiunta ai cheater: {ex}", LogTag);
//                }

//                try
//                {
//                    if (aggiungiBan && action == 2)
//                    {
//                        BanManager.AddBanPlayer(client, motivoBan);
//                        BMLogger.Info($"Giocatore aggiunto alla ban list: {nome} | ClientId={cheaterClientId}", LogTag);
//                    }
//                }
//                catch (Exception ex)
//                {
//                    BMLogger.Error($"Errore durante l'aggiunta alla ban list: {ex}", LogTag);
//                }
//            }

//            if (firstHandle && inviaWarning)
//            {
//                try
//                {
...
//                catch (Exception ex)
//                {
//                    BMLogger.Warn($"Impossibile inviare il warning globale anti-cheat: {ex.Message}", LogTag);
//                }
//            }
...
//                    BMLogger.Warn(
//                        $"{(action == 2 ? "Ban" : "Kick")} inviato al client | Giocatore={nome} | ClientId={cheaterClientId}",
//                        LogTag);
//                }
//                else
//                {
//                    BMLogger.Warn(
//                        $"Impossibile applicare il kick/ban: ClientId non valido ({cheaterClientId}) | Giocatore={nome}",
//                        LogTag);
//                }
//            }
//            catch (Exception ex)
//            {
//                BMLogger.Error($"Errore durante l'esecuzione del kick/ban per {nome} | ClientId={cheaterClientId}: {ex}", LogTag);
//            }
//        }
...
//            if (reader == null)
//            {
//                BMLogger.Warn($"RPC non valida: reader nullo | Giocatore={playerName} | CallId={callId}", LogTag);
//                return false;
//            }
...
//            {
//                BMLogger.Warn(
//                    $"RPC corrotta rilevata | Giocatore={playerName} | CallId={callId} | Position={reader.Position} | Length={reader.Length}",
//                    LogTag);
//                return false;
//            }
...
//            {
//                BMLogger.Error(
//                    $"RPC bloccata per dimensione eccessiva | Giocatore={playerName} | CallId={callId} | Dimensione={reader.Length} | Limite={MaxRpcDataSize}",
//                    LogTag);
//                return false;
//            }
...
//            {
//                case 101:
//                    BMLogger.Warn($"Rilevata RPC legacy sospetta 101 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "Legacy_Mod_Detected_101",
//                        $"{playerName} {GetActionLabel(action)} per mod legacy/HostGuard (RPC 101).");
//                    return true;

//                case 150:
//                    BMLogger.Warn($"Rilevata RPC sospetta 150 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "BetterAmongUs_RPC_150",
//                        $"{playerName} {GetActionLabel(action)} per BetterAmongUs (RPC 150).");
//                    return true;

//                case 176:
//                    BMLogger.Warn($"Rilevata RPC sospetta 176 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "HostGuard_RPC_176",
//                        $"{playerName} {GetActionLabel(action)} per HostGuard (RPC 176).");
//                    return true;

//                case 250:
//                    BMLogger.Warn($"Rilevata RPC sospetta 250 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "KillNetwork_RPC_250",
//                        $"{playerName} {GetActionLabel(action)} per KillNetwork (RPC 250).");
//                    return true;

//                case 420:
//                    BMLogger.Warn($"Rilevata RPC sospetta 420 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "SickoMenu_RPC_420",
//                        $"{playerName} {GetActionLabel(action)} per SickoMenu (RPC 420).");
//                    return true;

//                case 666:
//                    BMLogger.Warn($"Rilevata RPC sospetta 666 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "GoatNetClient_RPC_666",
//                        $"{playerName} {GetActionLabel(action)} per GoatNetClient (RPC 666).");
//                    return true;

//                case 42069:
//                    BMLogger.Warn($"Rilevata RPC sospetta 42069 da {playerName}.", LogTag);
//                    ApplicaAzioneSospetto(
//                        player, client, cheaterClientId, action,
//                        "AmongUsMenu_RPC_42069",
//                        $"{playerName} {GetActionLabel(action)} per AmongUsMenu (RPC 42069).");
//                    return true;
//            }
...
//        private static void BanForWrongModVersion(PlayerControl player, ClientData client, string remoteModInfo)
//        {
...
//            string playerName = NomeGiocatore(player);
//            string remoteVersion = NormalizeModVersion(remoteModInfo);

//            string reason = string.IsNullOrWhiteSpace(remoteVersion)
//                ? "Wrong_Mod_Version_Missing"
//                : $"Wrong_Mod_Version_{remoteVersion}_Expected_{CurrentModVersion}";

//            BMLogger.Warn(
//                $"Versione mod diversa rilevata | Player={playerName} | ClientId={clientId} | RemoteInfo={remoteModInfo} | RemoteVersion={remoteVersion} | Expected={CurrentModVersion}",
//                LogTag
//            );

//            try
//            {
//                ShowScreenMessage($"{playerName} bannato: versione BANMOD diversa.");
//            }
//            catch
//            {
//            }
...
//                    BMLogger.Warn($"Ban inviato per versione mod diversa | Player={playerName} | ClientId={clientId}", LogTag);
//                }
//            }
//            catch (Exception ex)
//            {
//                BMLogger.Error($"Errore ban versione diversa | Player={playerName} | ClientId={clientId} | {ex}", LogTag);
//            }
//        }
//        private static void DisconnectForWrongHostModVersion(string hostModInfo)
//        {
//            try
//            {
...
//                string hostVersion = NormalizeModVersion(hostModInfo);

//                BMLogger.Warn(
//                    $"Host con versione BANMOD incompatibile | HostInfo={hostModInfo} | HostVersion={hostVersion} | Expected={CurrentModVersion}",
//                    LogTag
//                );
...
//                {
//                    ShowScreenMessage(
//                        $"Lobby incompatibile: host BANMOD {hostVersion}, tua versione {CurrentModVersion}."
//                    );
//                }
//                catch
//                {
...
//            catch (Exception ex)
//            {
//                BMLogger.Error($"Errore durante disconnessione per versione host diversa: {ex}", LogTag);
//            }
//        }
...
//            if (__instance == null)
//            {
//                BMLogger.Warn("HandleRpc intercettata con PlayerControl nullo.", LogTag);
//                return true;
//            }

//            if (reader == null)
//            {
//                BMLogger.Warn($"HandleRpc intercettata con reader nullo | Giocatore={EtichettaGiocatore(__instance)} | CallIdInt={callId}", LogTag);
//                return false;
//            }
...
//            RegistraRpcRicevuta(__instance, callId, hasByteId, callIdByte, reader);

//            if (PlayerControl.LocalPlayer == __instance)
//                return true;

//            string name = NomeGiocatore(__instance);
...
//            try
//            {
//                if (HandleLegacyExploitRpc(__instance, client, cheaterClientId, action, callId, name))
//                    return false;

//                if (!hasByteId)
//                {
//                    BMLogger.Warn(
//                        $"RPC con CallId fuori range byte rilevata | Giocatore={name} | CallIdInt={callId}",
//                        LogTag);
...
//                        cheaterClientId,
//                        action,
//                        "RPC_CallId_Fuori_Range",
//                        $"{name} {GetActionLabel(action)} per RPC con identificatore non valido."
//                    );
...
//                        $"{name} {GetActionLabel(action)} per RPC malformata.",
//                        aggiungiBan: true,
//                        aggiungiCheater: true,
//                        inviaWarning: true
//                    );
...
//                {
//                    BMLogger.Warn(
//                        $"Flood totale RPC rilevato | Giocatore={name} | ClientId={cheaterClientId} | " +
//                        $"Conteggio={totalRpcCount} in {TotalRpcWindowSeconds:0.00}s",
//                        LogTag);
...
//                        $"{name} {GetActionLabel(action)} per flood RPC massivo."
//                    );
...
//                    LogSoloFile(
//                        $"Storico RPC | Giocatore={EtichettaGiocatore(__instance)} | CallIdInt={callId} | " +
//                        $"ConteggioFinestra={sameRpcCount} | Finestra={sameRpcWindow:0.00}s"
//                    );
...
//                            ? "abuso di PlayAnimation"
//                            : $"flood RPC {GetRpcNome(callId, true, callIdByte)}";

//                        BMLogger.Warn(
//                            $"Flood RPC specifica rilevato | Giocatore={name} | CallId={callId} | Conteggio={sameRpcCount}",
//                            LogTag);
...
//                            action,
//                            reasonCode,
//                            $"{name} {GetActionLabel(action)} per {reasonText}.",
//                            aggiungiBan: true,
//                            aggiungiCheater: true,
//                            inviaWarning: true
//                        );
...
//                LogSoloFile(
//                    $"Dettaglio RPC | Giocatore={EtichettaGiocatore(__instance)} | NomeRpc={GetRpcNome(callId, true, callIdByte)}"
//                );
...
//                    !__instance.AmOwner)
//                {
//                    BMLogger.Warn($"Bloccata RPC Exiled fuori meeting da {name}.", LogTag);
...
//                        action,
//                        "Crash_RPC_Exiled",
//                        $"{name} {GetActionLabel(action)} per RPC Exiled sospetta."
//                    );
...
//                    MeetingHud.Instance == null)
//                {
//                    BMLogger.Warn($"Bloccata RPC VotingComplete in lobby da {name}.", LogTag);
...
//                        $"{name} {GetActionLabel(action)} per VotingComplete sospetta in lobby."
//                    );
...
//                    catch (Exception ex)
//                    {
//                        BMLogger.Warn($"CustomHatSync RPC 240 non valida da {name}: {ex.Message}", LogTag);
//                    }
...
//                {
//                    LogSoloFile(
//                        $"RPC sconosciuta ricevuta | Giocatore={EtichettaGiocatore(__instance)} | CallIdInt={callId} | CallIdByte={callIdByte}"
//                    );
...
//                        action,
//                        "RPC_Sconosciuta",
//                        $"{name} {GetActionLabel(action)} per RPC sconosciuta ({callIdByte})."
//                    );
...
//                    LogSoloFile($"Gestione CustomRPC | Giocatore={EtichettaGiocatore(__instance)} | Tipo={customType}");
...
//                                LogSoloFile($"HandshakeModded registrato per {EtichettaGiocatore(__instance)}.");
//                                return false;
//                            }
...
//                                LogSoloFile(
//                                    $"ModdedHandshake ricevuta | Giocatore={EtichettaGiocatore(__instance)} | ModInfo={PulisciTestoLog(cleanModInfo)} | FlagModded={isRemoteModded}"
//                                );

//                                if (AmongUsClient.Instance != null &&
//                                    AmongUsClient.Instance.AmHost &&
//                                    isRemoteModded &&
//                                    !IsHostPlayer(__instance))
//                                {
//                                    if (!IsSameModVersion(cleanModInfo))
//                                    {
//                                        BanForWrongModVersion(__instance, client, cleanModInfo);
//                                        return false;
//                                    }
//                                }
...
//                                    if (!IsSameModVersion(cleanModInfo))
//                                    {
//                                        DisconnectForWrongHostModVersion(cleanModInfo);
//                                        return false;
//                                    }
//                                }
...
//                                        $"Client moddato connesso: {NomeGiocatore(__instance)} | Mod: {cleanModInfo}"
//                                    );
//                                }
...
//                                {
//                                    LogSoloFile(
//                                        $"HostTripleBoolUpdate BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");
...
//                                LogSoloFile(
//                                    $"HostTripleBoolUpdate ricevuta | Added={remoteBool1ImmortalAdded} | Enabled={remoteBool2ImmortalEnabled} | Engineer={remoteBool3EngineerEnabled}"
//                                );
...
//                                {
//                                    LogSoloFile(
//                                        $"ProxySendChat BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");

//                                    return false;
//                                }
//                                string message = subReader.ReadString();
//                                string titoloIgnorato = subReader.ReadString();
//                                byte target = subReader.ReadByte();

//                                RegistraMessaggioRicevuto(__instance, "ProxySendChat", message);

//                                LogSoloFile(
//                                    $"ProxySendChat ricevuto | TitoloIgnorato={PulisciTestoLog(titoloIgnorato)} | TargetPlayerId={target}");
...
//                                        LogSoloFile(
//                                            $"ProxySendChat mostrato localmente al proxy stesso | Proxy={EtichettaGiocatore(localPlayer)} | TargetPlayerId={target} | Testo={PulisciTestoLog(message)}");
//                                    }
//                                    catch (Exception ex)
//                                    {
//                                        BMLogger.Warn($"ProxySendChat AddChat locale fallito: {ex}", LogTag);
//                                    }
...
//                                    LogSoloFile(
//                                        $"ProxySendChat accodata per cooldown | TargetPlayerId={target} | TargetClientId={targetClientId} | SendToAll={sendToAll} | Testo={PulisciTestoLog(message)}");
...
//                                        catch (Exception ex)
//                                        {
//                                            BMLogger.Warn($"ProxySendChat AddChat broadcast locale fallito: {ex}", LogTag);
//                                        }
//                                    }
...
//                                    LogSoloFile(
//                                        $"ProxySendChat inviato | Proxy={EtichettaGiocatore(localPlayer)} | TargetPlayerId={target} | TargetClientId={targetClientId} | SendToAll={sendToAll} | Testo={PulisciTestoLog(message)}");
//                                }
//                                catch (Exception ex)
//                                {
//                                    BMLogger.Warn($"ProxySendChat invio fallito: {ex}", LogTag);
//                                }
...
//                                RegistraMessaggioRicevuto(__instance, "ModdedAllChat", text);

//                                if (IsChatDangerous(text, true))
//                                {
//                                    BMLogger.Warn(
//                                        $"Messaggio ModdedAllChat pericoloso bloccato da {name}: {PulisciTestoLog(text)}",
//                                        LogTag);
...
//                                {
//                                    LogSoloFile(
//                                        $"HostRoleOptionsUpdate BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");
...
//                                {
//                                    LogSoloFile(
//                                        $"SetSpecialKiller BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");
...
//                                LogSoloFile($"SetSpecialKiller ricevuta | KillerId={killerId}");
...
//                                {
//                                    LogSoloFile(
//                                        $"SetJester BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");
...
//                                LogSoloFile($"SetJester ricevuta | JesterId={jesterId}");
...
//                                {
//                                    LogSoloFile(
//                                        $"SetExiler BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");
...
//                                LogSoloFile($"SetExiler ricevuta | ExilerId={exilerId}");
...
//                                {
//                                    LogSoloFile(
//                                        $"SetWatcher BLOCCATO: mittente non host | Sender={EtichettaGiocatore(sender)} | SenderPlayerId={sender?.PlayerId} | SenderClientId={sender?.GetClientId()} | HostId={AmongUsClient.Instance?.HostId}");
...
//                                LogSoloFile($"SetWatcher ricevuta | WatcherId={watcherId}");
...
//                                    $"{name} {GetActionLabel(action)} per SickoMenu."
//                                );
...
//                                    $"{name} {GetActionLabel(action)} per CREWMODIMPOSTORFORCE."
//                                );
...
//                                    $"{name} {GetActionLabel(action)} per KillNetWork Hack."
//                                );
...
//                LogSoloFile($"Gestione RpcCalls standard | Giocatore={EtichettaGiocatore(__instance)} | Tipo={rpcType}");
...
//                        {
//                            string text = subReader.ReadString();
//                            RegistraMessaggioRicevuto(__instance, "SendChat", text);
...
//                            {
//                                BMLogger.Warn(
//                                    $"Chat flood rilevato | Giocatore={name} | ClientId={cheaterClientId} | Conteggio={chatCount} in {ChatWindowSeconds:0.0}s",
//                                    LogTag);
...
//                                    action,
//                                    "Chat_Flood",
//                                    $"{name} {GetActionLabel(action)} per chat flood."
//                                );
...
//                            {
//                                BMLogger.Warn($"Messaggio chat pericoloso bloccato da {name}: {PulisciTestoLog(text)}", LogTag);
...
//                                    $"{name} {GetActionLabel(action)} per messaggio chat pericoloso."
//                                );
...
//                            if (canceled)
//                            {
//                                LogSoloFile($"Messaggio chat annullato dai comandi | Giocatore={EtichettaGiocatore(__instance)}");
//                                return false;
//                            }
...
//                        {
//                            RegistraMessaggioRicevuto(__instance, "SendQuickChat", "Quick chat");
//                            ChatCommands.OnReceiveChat(__instance, "Quick chat", out var canceledQuick);

//                            if (canceledQuick)
//                            {
//                                LogSoloFile($"Quick chat annullata dai comandi | Giocatore={EtichettaGiocatore(__instance)}");
//                                return false;
//                            }
...
//            {
//                BMLogger.Error(
//                    $"Errore durante la gestione RPC | Giocatore={name} | CallId={callId} | Errore={ex}",
//                    LogTag);
...
//                    action,
//                    "RPC_Exception",
//                    $"{name} {GetActionLabel(action)} per RPC malformata o crash attempt."
//                );
...
//                catch (Exception ex)
//                {
//                    BMLogger.Warn($"Impossibile ripristinare la posizione del reader: {ex.Message}", LogTag);
//                }
//            }
//        }
...
//            {
//                BMLogger.Error("Exiled() chiamato in lobby. Blocco di sicurezza applicato.", "RPCHandler");
//                return false;
//            }
...
namespace BanMod
{
    ...
    internal class UnifiedRPCHandlerPatch
    {
        private const string CurrentModVersion = BanMod.PluginVersion;
        private const string LogTag = "RPCHandler";
        private const int MaxRpcDataSize = 128;
